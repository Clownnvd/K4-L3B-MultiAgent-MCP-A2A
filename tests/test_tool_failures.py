...
def test_server_error_is_typed_but_invalid_contract_is_not():
    ...
    with pytest.raises(ValueError):
        asyncio.run(
            EvidenceGateway(InvalidSession(), contracts).call("get_order", case_id="CASE_001")
        )
...
def test_entity_repair_removes_unsupported_rejection_without_new_tool_calls(tmp_path):
    class MissingCandidate(DemoGateway):
        async def call(self, tool_name, *, case_id, **arguments):
            if tool_name == "get_order" and arguments["order_id"].startswith("wrong-"):
                raise MCPToolError(tool_name, "execution error")
    ...
    class RepairModel(DemoModel):
        ...
        async def complete(self, task, payload):
            ...
            if task == "resolve_entity":
                self.attempts += 1
                if self.attempts == 1:
                    result["rejected_candidates"] = ["wrong-order-0001"]
                else:
                    assert "Unreadable candidate" in payload["validation_feedback"]
            return result

    contracts, trace = setup_trace(tmp_path)
    case = demo_cases(1)[0]
    model = RepairModel()
    gateway = MissingCandidate([case])
    output = asyncio.run(Orchestrator(contracts, model).solve(case, gateway, trace))
    assert model.attempts == 2
    assert output["entity_resolution"]["rejected_candidates"] == []
    assert sum(tool == "get_order" for tool, _ in gateway.calls) == 1
                    ...
                    "calculations": [
                        {
                            ...
                            "operation": "sum",
                            "operands": [
                                {"evidence_ref": payment_ref, "pointer": "/captured_total_brl"}
                            ],
                        }
                    ],
                }
