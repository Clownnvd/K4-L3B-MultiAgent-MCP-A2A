...
def main() -> None:
    ...
    initialized = rpc(
        "initialize",
        {
            ...
            "capabilities": {},
            "clientInfo": {"name": "kingpro-inspection", "version": "0.1.0"},
        },
    )
    ...
    order_id = (
        cases.get("customer_request", {}).get("claimed_order_id") or cases["candidate_order_ids"][0]
    )
    selected = (
        tools if args.all_tools else [t for t in tools if t["name"] in {"get_order", "get_policy"}]
    )
    results = {}
    for tool in selected:
        ...
        if "customer_unique_id" in props:
            ...
            parameters["customer_unique_id"] = (
                order_data.get("customer_unique_id") or cases["customer_unique_id_hint"]
            )
